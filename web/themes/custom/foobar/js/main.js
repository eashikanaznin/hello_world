...
(function ($) {
  /*load more for front page faq*/
  $(".faq-load").on("click", function () {
    var tar_id = $(this).closest('.tab-pane').attr('id');
    var count = $("#" + tar_id + " .card").length;
    var countHidden = $("#" + tar_id + " .card.visually-hidden").length;
    var itemsToShow = (count - countHidden) + 5;
    if (count > countHidden) {
      $('#' + tar_id + ' .card').slice(0, itemsToShow).removeClass("visually-hidden");
    }
    var countHidden = $("#" + tar_id + " .card.visually-hidden").length;
    if (countHidden < 1) {
      $(this).hide();
  ...
  $(".tab-content .tab-pane").first().addClass("show active content-tween");
})(jQuery);
